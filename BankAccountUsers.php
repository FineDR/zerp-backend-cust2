...
if (isset($_POST['Cancel'])) {
	unset($SelectedBankAccount, $SelectedUser);
}
...
if (isset($_POST['submit'])) {
	if ($_POST['SelectedUser']=='') {
		...
		if ($CheckRow[0] > 0) {
			...
			prnMsg(__('User authorised successfully'),'success');
	...
	prnMsg(__('User access revoked'),'success');
}
...
if (!isset($SelectedBankAccount)) {
	...
	echo '<div class="db-card" style="max-width:600px; margin:0 auto;"><div class="db-card-header"><i class="fas fa-university" style="color:var(--db-primary)"></i><h3 class="db-card-title">' . __('Account Selection') . '</h3></div>';
    ...
    echo '<div class="db-form-group"><label class="db-label">Select Bank Account</label><select name="SelectedBankAccount" class="db-select" onchange="this.form.submit()"><option value="">' . __('Select account...') . '</option>';
    $Result = DB_query("SELECT accountcode, bankaccountname, currcode FROM bankaccounts ORDER BY accountcode");
    while ($MyRow = DB_fetch_array($Result)) echo '<option value="'.$MyRow['accountcode'].'">'.$MyRow['accountcode'].' - '.$MyRow['bankaccountname'].' ('.$MyRow['currcode'].')</option>';
    echo '</select></div><button name="Process" type="submit" class="db-btn db-btn-primary"><i class="fas fa-key"></i> ' . __('Manage Authorisations') . '</button></form></div></div>';
} else {
    $MyRow = DB_fetch_array(DB_query("SELECT bankaccountname FROM bankaccounts WHERE accountcode='".$SelectedBankAccount."'"));
    $SelectedBankName = $MyRow['bankaccountname'];

    echo '<header class="db-header"><div class="db-breadcrumb"><a href="'.basename(__FILE__).'" style="color:inherit">'.__('Banking Security').'</a> / '.$SelectedBankAccount.'</div><h1 class="db-title">'.$SelectedBankName.'</h1></header>';
    
    echo '<div class="db-layout">';
    
    // MAIN: Users List
    echo '<main class="db-main">';
    echo '<div class="db-card"><div class="db-card-header"><i class="fas fa-users-cog" style="color:var(--db-primary)"></i><h3 class="db-card-title">' . __('Authorised Personnel') . '</h3></div>';
    echo '<div style="overflow-x:auto;"><table class="db-table"><thead><tr><th>User ID</th><th>Full Name</th><th style="text-align:right">Actions</th></tr></thead><tbody>';
    $Result = DB_query("SELECT bankaccountusers.userid, www_users.realname FROM bankaccountusers INNER JOIN www_users ON bankaccountusers.userid=www_users.userid WHERE bankaccountusers.accountcode='".$SelectedBankAccount."' ORDER BY bankaccountusers.userid ASC");
    if (DB_num_rows($Result) > 0) {
        while ($MyRow = DB_fetch_array($Result)) {
            echo '<tr><td style="font-weight:700;">'.$MyRow['userid'].'</td><td>'.$MyRow['realname'].'</td><td style="text-align:right;"><a class="db-btn db-btn-outline" style="padding:0.4rem 0.6rem; color:#dc2626; width:auto;" href="'.basename(__FILE__).'?SelectedUser='.$MyRow['userid'].'&delete=yes&SelectedBankAccount='.$SelectedBankAccount.'" onclick="return confirm(\''.__('Revoke access?').'\');"><i class="fas fa-user-minus"></i></a></td></tr>';
        }
    } else {
        echo '<tr><td colspan="3" style="text-align:center; padding:2rem; color:#64748b;">'.__('No authorised users found for this account.').'</td></tr>';
    }
    echo '</tbody></table></div></div></main>';

    // SIDEBAR: Add User
    echo '<aside class="db-aside">';
    echo '<div class="db-card"><div class="db-card-header"><i class="fas fa-user-plus" style="color:var(--db-primary)"></i><h3 class="db-card-title">' . __('Authorise User') . '</h3></div>';
    echo '<div class="db-card-body"><form method="post" action="'.basename(__FILE__).'"><input type="hidden" name="FormID" value="'.$_SESSION['FormID'].'" /><input type="hidden" name="SelectedBankAccount" value="'.$SelectedBankAccount.'" />';
    echo '<div class="db-form-group"><label class="db-label">Target User</label><select name="SelectedUser" class="db-select"><option value="">' . __('Select user...') . '</option>';
    $UR = DB_query("SELECT userid, realname FROM www_users WHERE NOT EXISTS (SELECT userid FROM bankaccountusers WHERE accountcode='".$SelectedBankAccount."' AND bankaccountusers.userid=www_users.userid) ORDER BY userid");
    while ($r = DB_fetch_array($UR)) echo '<option value="'.$r['userid'].'">'.$r['userid'].' - '.$r['realname'].'</option>';
    ...
    echo '<a class="db-btn db-btn-outline" href="'.basename(__FILE__).'" style="width:100%"><i class="fas fa-reply-all"></i> Switch Account</a>';
