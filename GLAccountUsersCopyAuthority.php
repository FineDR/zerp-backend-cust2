...
$Title = __('Copy Authority of GL Accounts');
...
if (isset($_POST['ProcessCopyAuthority'])) {
	...
	if ($InputError == 0) {
		...
		DB_query($SQL, '', '', true);
		prnMsg(__('Cleared previous authority for user') . ' ' . $_POST['ToUserID'], 'success');
		...
		DB_query($SQL, '', '', true);
		prnMsg(__('Copied GL authority from') . ' ' . $_POST['FromUserID'] . ' ' . __('to') . ' ' . $_POST['ToUserID'], 'success');
		DB_Txn_Commit();
	}
}
...
echo '<header class="db-header"><div class="db-breadcrumb">' . __('General Ledger') . ' / ' . __('Tools') . '</div><h1 class="db-title">' . $Title . '</h1></header>';

echo '<div class="db-card">
        <div class="db-card-header"><i class="fas fa-users-cog" style="color:var(--db-primary); font-size:1.5rem;"></i><h3 class="db-card-title">' . __('Authority Migration Wizard') . '</h3></div>
        <div class="db-card-body">
            <form action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') .  '" method="post">
            <input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />
            
            <div class="db-form-group">
                <label class="db-label">' . __('Source User (Copy FROM)') . '</label>
...
echo '          </select>
            ...
            <div class="db-form-group">
                <label class="db-label">' . __('Destination User (Copy TO)') . '</label>
                <select name="ToUserID" class="db-select" required>
                    <option value="">' . __('Select target user...') . '</option>';
                    DB_data_seek($users, 0);
                    while($r = DB_fetch_array($users)) echo '<option value="'.$r['userid'].'">'.$r['userid'].' - '.$r['realname'].'</option>';
echo '          </select>
                <div style="font-size:0.7rem; color:#dc2626; margin-top:0.5rem; font-weight:600;">* ' . __('WARNING: Previous permissions for the target user will be overwritten.') . '</div>
            </div>

            <button type="submit" name="ProcessCopyAuthority" class="db-btn db-btn-primary">
                <i class="fas fa-exchange-alt"></i> ' . __('Execute Permission Transfer') . '
            </button>
            </form>
        </div>
      </div>';

echo '<div style="margin-top:2rem;"><a href="index.php?Application=GL" style="text-decoration:none; color:var(--db-primary); font-weight:700; font-size:0.875rem;"><i class="fas fa-reply"></i> ' . __('Return to GL Menu') . '</a></div>';
