...
# 1. Replace the CSS block exactly.
...
# 2. Remove the step indicator
step_ind_start = content.find("echo '<div class=\"step-indicator\">")
step_ind_end = content.find("</div>';", step_ind_start) + len("</div>';")
if step_ind_start != -1:
    content = content[:step_ind_start] + content[step_ind_end:]

# 3. Replace the form body
...
new_body = """echo '<div class="db-bottom-layout">';
    ...
    $ActiveStep = 'step1';
    if (count($_SESSION['SuppTrans']->GRNs) > 0 || count($_SESSION['SuppTrans']->GLCodes) > 0 || count($_SESSION['SuppTrans']->Shipts) > 0 || count($_SESSION['SuppTrans']->Contracts) > 0 || count($_SESSION['SuppTrans']->Assets) > 0) {
        $ActiveStep = 'step2';
    }
