...
$Title = __('Horizontal Analysis: Financial Position');
$ViewTopic = 'GeneralLedger';
$BookMark = 'AnalysisHorizontalPosition';
include(__DIR__ . '/includes/header.php');

// Merges gets into posts:
foreach(array('PeriodTo','ShowDetail','ShowZeroBalance','NewReport') as $f) if(isset($_GET[$f])) $_POST[$f] = $_GET[$f];
...
echo '<style>
    ...
    .report-table th { background: hsl(197, 75%, 22%); color: white; padding: 12px; text-align: left; font-size: 0.7rem; text-transform: uppercase; }
    .report-table td { padding: 10px; border-bottom: 1px solid #f1f5f9; }
    .var-pos { color: hsl(197, 92%, 47%); font-weight: 700; }
    .var-neg { color: #dc2626; font-weight: 700; }
    @media print { .no_print { display: none; } .db-page { padding: 0; background: white; } }
</style>';

if (isset($_POST['PeriodTo']) and (!isset($_POST['NewReport']) OR $_POST['NewReport']!='on')) {
    
    $BalDate = MonthAndYearFromSQLDate(EndDateSQLFromPeriodNo($_POST['PeriodTo']));
    $RetEarnAct = $_SESSION['CompanyRecord']['retainedearnings'];

    $AccumProf = DB_fetch_array(DB_query("SELECT SUM(CASE WHEN gltotals.period <= '" . $_POST['PeriodTo'] . "' THEN gltotals.amount ELSE 0 END) AS bfwd, SUM(CASE WHEN gltotals.period <= '" . ($_POST['PeriodTo'] - 12) . "' THEN gltotals.amount ELSE 0 END) AS bfwdly FROM chartmaster INNER JOIN accountgroups ON chartmaster.group_ = accountgroups.groupname INNER JOIN gltotals ON chartmaster.accountcode = gltotals.account WHERE accountgroups.pandl = 1"));
    ...
    echo '<div style="color:#64748b; font-size:0.85rem;">' . __('As at') . ' ' . $BalDate . '</div>';
    echo '</div>';

    echo '<div class="db-card-body" style="padding:0;"><table class="report-table"><thead><tr>';
    if ($_POST['ShowDetail'] == 'Detailed') echo '<th>Account</th><th>Account Name</th>';
    else echo '<th colspan="2">Summary Grouping</th>';
    ...
    $SQL = "SELECT accountgroups.sectioninaccounts, accountgroups.groupname, accountgroups.parentgroupname, gltotals.account AS accountcode, chartmaster.accountname, SUM(CASE WHEN gltotals.period <= '" . $_POST['PeriodTo'] . "' THEN gltotals.amount ELSE 0 END) AS balancecfwd, SUM(CASE WHEN gltotals.period <= '" . ($_POST['PeriodTo'] - 12) . "' THEN gltotals.amount ELSE 0 END) AS balancecfwdly FROM chartmaster INNER JOIN accountgroups ON chartmaster.group_ = accountgroups.groupname INNER JOIN gltotals ON chartmaster.accountcode = gltotals.account INNER JOIN glaccountusers ON glaccountusers.accountcode = chartmaster.accountcode AND glaccountusers.userid = '" . $_SESSION['UserID'] . "' AND glaccountusers.canview = 1 WHERE accountgroups.pandl = 0 GROUP BY accountgroups.groupname, gltotals.account, chartmaster.accountname, accountgroups.parentgroupname, accountgroups.sequenceintb, accountgroups.sectioninaccounts ORDER BY accountgroups.sectioninaccounts, accountgroups.sequenceintb, accountgroups.groupname, gltotals.account";
    $Res = DB_query($SQL);

	$Section = ''; $SectionBal = 0; $SectionBalLY = 0; $CheckTotal = 0; $CheckTotalLY = 0; $ActGrp = ''; $Level = 0; $GrpTotal = array(0); $GrpTotalLY = array(0); $ParentGroups = array();

	while ($MyRow = DB_fetch_array($Res)) {
		$AccBal = $MyRow['balancecfwd']; $AccBalLY = $MyRow['balancecfwdly'];
		if ($MyRow['accountcode'] == $RetEarnAct) { $AccBal += $AccumProf['bfwd']; $AccBalLY += $AccumProf['bfwdly']; }
		...
		if ($MyRow['sectioninaccounts'] != $Section) {
			if ($Section != '') {
				echo '<tr style="background:hsl(197, 65%, 95%); font-weight:900;"><td colspan="2">' . $Sections[$Section] . ' Total</td><td style="text-align:right;">' . locale_number_format($SectionBal, 2) . '</td><td style="text-align:right;">' . locale_number_format($SectionBalLY, 2) . '</td><td style="text-align:right;">' . locale_number_format($SectionBal-$SectionBalLY, 2) . '</td><td style="text-align:right;">' . RelativeChange($SectionBal, $SectionBalLY) . '</td></tr>';
			}
			$Section = $MyRow['sectioninaccounts']; $SectionBal = 0; $SectionBalLY = 0;
			if ($_POST['ShowDetail'] == 'Detailed') echo '<tr style="background:#f8fafc; font-weight:900; color:var(--db-primary-dark);"><td colspan="6">' . $Sections[$Section] . '</td></tr>';
		}
		...
		for ($i = 0; $i <= $Level; $i++) { if(!isset($GrpTotal[$i]))$GrpTotal[$i]=0; $GrpTotal[$i]+=$AccBal; if(!isset($GrpTotalLY[$i]))$GrpTotalLY[$i]=0; $GrpTotalLY[$i]+=$AccBalLY; }
		$SectionBal += $AccBal; $SectionBalLY += $AccBalLY; $CheckTotal += $AccBal; $CheckTotalLY += $AccBalLY;

		if ($_POST['ShowDetail'] == 'Detailed') {
			if (isset($_POST['ShowZeroBalance']) OR (round($AccBal, 2) != 0 OR round($AccBalLY, 2) != 0)) {
				$var = $AccBal - $AccBalLY; $rel = RelativeChange($AccBal, $AccBalLY);
				echo '<tr><td style="padding-left:' . (25+($Level*15)) . 'px;">' . $MyRow['accountcode'] . '</td><td>' . htmlspecialchars($MyRow['accountname']) . '</td><td style="text-align:right;">' . locale_number_format($AccBal, 2) . '</td><td style="text-align:right;">' . locale_number_format($AccBalLY, 2) . '</td><td style="text-align:right;' . ($var < 0 ? 'color:#dc2626;' : 'color:hsl(197, 92%, 47%);') . ' font-weight:600;">' . locale_number_format($var, 2) . '</td><td style="text-align:right;">' . $rel . '</td></tr>';
			}
		}
	}
	echo '<tr style="background:var(--db-primary); color:white; font-weight:900;">' . '<td colspan="2">' . __('CHECK TOTAL') . '</td><td style="text-align:right;">' . locale_number_format($CheckTotal, 2) . '</td><td style="text-align:right;">' . locale_number_format($CheckTotalLY, 2) . '</td><td style="text-align:right;">' . locale_number_format($CheckTotal - $CheckTotalLY, 2) . '</td><td style="text-align:right;">' . RelativeChange($CheckTotal, $CheckTotalLY) . '</td></tr>';
	echo '</tbody></table></div></div>';
    echo '<div class="no_print" style="display:flex; justify-content:center; gap:15px; margin-top:2rem;">
    ...
    echo '<div class="db-page"><div class="db-card" style="max-width:600px; margin:0 auto;">
            <div class="db-card-header"><h3 class="db-card-title">' . __('Horizontal Balance Sheet Trend') . '</h3></div>
            <div class="db-card-body">
                <form action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '" method="post">
                <input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />
                
                <div class="db-field"><label class="db-label">Target Balance Date</label><select name="PeriodTo" class="db-select">';
                ...
                <div class="db-field"><label class="db-label">Report Depth</label><select name="ShowDetail" class="db-select"><option value="Detailed">Include G/L Accounts</option><option value="Summary">Group Totals Only</option></select></div>
                ...
                <button type="submit" class="db-btn db-btn-primary">Generate Position Analysis</button>
                </form>
            </div>
        </div></div>';
}
