...
# 1. Replace the CSS block (lines 686 to 784) and the step indicator
css_pattern = re.compile(r"echo '<style>.*?echo '<div class=\"step-indicator\">.*?</div>\s*</div>';", re.DOTALL)
...
<script>
    function suppToggleStep(stepId) {
        document.querySelectorAll(".pay-section").forEach(function(el) {
            el.classList.remove("active");
        });
        document.getElementById(stepId).classList.add("active");
    }
    function suppNextStep(stepId) {
        suppToggleStep(stepId);
...
# 2. Replace the main form content (from '<div class="db-bottom-layout">' to '<!-- .db-page -->')
# We need to find the form body start and end.
...
if form_body_start != -1 and form_body_end != -1:
    old_body = content[form_body_start:form_body_end]
    ...
    // Determine initial active step based on state
    $ActiveStep = 'step1';
    if (count($_SESSION['SuppTrans']->GRNs) > 0 || count($_SESSION['SuppTrans']->GLCodes) > 0) {
        $ActiveStep = 'step2';
    }
    // If user clicked 'Review', set to step 3
    if (isset($_POST['GoToReview'])) {
        $ActiveStep = 'step3';
    }
    ...
    // -------------------------------------------------------------
    // ACCORDION STEP 1: HEADER
    // -------------------------------------------------------------
    echo '<div id="step1" class="pay-section ' . ($ActiveStep == 'step1' ? 'active' : '') . '">
            <div class="pay-section-header-banner" onclick="suppToggleStep(\'step1\')">
                <div class="pay-step-number">1</div>
                <div class="pay-step-title">' . __('Setup Invoice Header') . '</div>
            </div>
    ...
    echo '</div>
            <div style="margin-top: 30px; text-align: right;">
                <button type="button" onclick="suppNextStep(\'step2\')" class="architect-btn" style="padding: 15px 40px; font-size: 1.1rem; border-radius: 12px;">
                    ' . __('Continue to Allocations') . ' <i class="fas fa-arrow-right" style="margin-left: 8px;"></i>
                </button>
            </div>
        </div>
    </div>';
    ...
    // -------------------------------------------------------------
    // ACCORDION STEP 2: CHARGES / GL ALLOCATION
    // -------------------------------------------------------------
    echo '<div id="step2" class="pay-section ' . ($ActiveStep == 'step2' ? 'active' : '') . '">
            <div class="pay-section-header-banner" onclick="suppToggleStep(\'step2\')">
                <div class="pay-step-number">2</div>
                <div class="pay-step-title">' . __('Analysis & Allocation') . '</div>
            </div>
            <div class="pay-section-body">';

    echo '      <div class="db-grid db-grid-3" style="margin-bottom:30px;">
    ...
    echo '      <div style="margin-top: 30px; text-align: right;">
                    <button type="button" onclick="suppNextStep(\'step3\')" class="architect-btn" style="padding: 15px 40px; font-size: 1.1rem; border-radius: 12px;">
                        ' . __('Continue to Review') . ' <i class="fas fa-arrow-right" style="margin-left: 8px;"></i>
                    </button>
                </div>
            </div>
          </div>';
    ...
    // -------------------------------------------------------------
    // ACCORDION STEP 3: REVIEW & FINISH
    // -------------------------------------------------------------
    echo '<div id="step3" class="pay-section ' . ($ActiveStep == 'step3' ? 'active' : '') . '">
            <div class="pay-section-header-banner" onclick="suppToggleStep(\'step3\')">
                <div class="pay-step-number">3</div>
                <div class="pay-step-title">' . __('Review & Post') . '</div>
            </div>
            <div class="pay-section-body">
...
"""
    ...
    print("Could not find start or end markers for the form body.")
